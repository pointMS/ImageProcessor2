...
public class ActionsConfigService {
    ...
    private void loadProperties() {
        try (InputStream input = ImageActionFactory.class.getClassLoader().getResourceAsStream("actions.properties")) {
            if (input == null) {
                System.out.println("Sorry, unable to find actions.properties");
            }
            //load a properties file from class path, inside of static method
            prop.load(input);
        } catch (IOException e) {
            e.printStackTrace();
        }
